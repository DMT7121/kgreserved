...
<template>
  <div :class="['w-full md:w-5/12 glass-panel flex flex-col shadow-[0_0_40px_rgba(0,0,0,0.5)] z-20 border-r border-glass text-[13px] safe-area-pt', ui.tab === 'preview' ? 'h-auto shrink-0 md:h-full md:shrink' : 'h-full flex-1 md:flex-none']">
    <!-- HEADER -->
    <div class="flex-shrink-0 bg-transparent border-b border-[rgba(255,255,255,0.1)] p-4 flex justify-between items-center">
    ...
    <!-- TABS -->
    <div class="flex bg-[rgba(15,23,42,0.4)] text-[11px] font-black border-b border-[rgba(255,255,255,0.1)] uppercase tracking-widest backdrop-blur-md">
      <button @click="ui.tab = 'create'" :class="['flex-1 py-4 flex justify-center gap-2 transition-all min-h-[44px]', ui.tab === 'create' ? 'text-indigo-400 border-b-4 border-indigo-500 bg-[rgba(99,102,241,0.1)] text-shadow-[0_0_10px_rgba(99,102,241,0.5)]' : 'text-slate-400']" :style="ui.tab === 'create' ? { color: configStore.branding.color, borderColor: configStore.branding.color, backgroundColor: configStore.branding.color + '1a' } : {}"><i class="fa-solid fa-pen-nib"></i> Nhập Liệu</button>
      <button @click="appStore.loadHistory(false)" :class="['flex-1 py-4 flex justify-center gap-2 transition-all min-h-[44px]', ui.tab === 'history' ? 'text-indigo-400 border-b-4 border-indigo-500 bg-[rgba(99,102,241,0.1)] text-shadow-[0_0_10px_rgba(99,102,241,0.5)]' : 'text-slate-400']" :style="ui.tab === 'history' ? { color: configStore.branding.color, borderColor: configStore.branding.color, backgroundColor: configStore.branding.color + '1a' } : {}"><i class="fa-solid fa-clock-rotate-left"></i> Lịch Sử</button>
      <button @click="ui.tab = 'preview'" class="md:hidden flex-1 py-4 flex justify-center gap-2 text-slate-300 bg-[rgba(255,255,255,0.05)] min-h-[44px]"><i class="fa-solid fa-eye"></i> Preview</button>
    </div>

    <!-- CREATE TAB -->
    <div v-show="ui.tab === 'create'" class="flex-grow overflow-y-auto p-4 md:p-5 space-y-4 pb-28 md:pb-6 bg-transparent scroll-smooth custom-scrollbar">
      <AIInputPanel />
      <div class="space-y-4">
        <CustomerForm />
        <DepositManager />
    ...
    <div v-show="ui.tab === 'create' && !ui.isKeyboardOpen" class="hidden md:grid grid-cols-4 gap-3 p-5 border-t border-[rgba(255,255,255,0.1)] bg-[rgba(15,23,42,0.6)] backdrop-blur-md z-20">
    ...
    <!-- MOBILE FOOTER -->
    <div v-if="ui.tab === 'create' && !ui.isKeyboardOpen" class="md:hidden fixed bottom-0 left-0 right-0 p-4 bg-[rgba(15,23,42,0.8)] backdrop-blur-xl border-t border-[rgba(255,255,255,0.1)] z-40 grid grid-cols-4 gap-2 shadow-2xl safe-area-pb">
      <button @click="doSave('save')" class="bg-emerald-600 text-white py-4 rounded-2xl font-black flex flex-col items-center text-[9px] gap-1 shadow-[0_0_15px_rgba(5,150,105,0.4)] active:scale-95 transition-transform uppercase tracking-tighter min-h-[60px] active-effect hover-effect"><i class="fa-solid fa-cloud-arrow-up text-xl"></i> LƯU</button>
